...
from exp_crossformer import Exp_crossformer
from tools import string_split

parser = argparse.ArgumentParser(description='CrossFormer')

parser.add_argument('--data', type=str, required=True, default='Data', help='data')
parser.add_argument('--root_path', type=str, default='./datasets/', help='root path of the data file')
parser.add_argument('--data_path', type=str, default='Data2.csv', help='data file')
...
parser.add_argument('--in_len', type=int, default=96, help='input Data length (T)')
parser.add_argument('--out_len', type=int, default=24, help='output Data length (\tau)')
...
parser.add_argument('--data_dim', type=int, default=6, help='Number of dimensions of the MTS data (D)')
parser.add_argument('--d_model', type=int, default=1024, help='dimension of hidden states (d_model)')
parser.add_argument('--d_ff', type=int, default=2028, help='dimension of MLP in transformer')
...
data_parser = {
    'Data': {'data': 'Data.csv', 'data_dim': 6, 'split': [0.7, 0.1, 0.2]},
}
...
for ii in range(args.itr):
    ...
    setting = 'Crossformer_{}__d_model{}__n_heads{}__d_ff{}__batch_size{}__learning_rate{}'.format(args.data,
                                                                                     args.d_model, args.n_heads,
                                                                                     args.d_ff, args.batch_size,
                                                                                     args.learning_rate
                                                                                    )
