from data_loader import Dataset_MTS
...
class Exp_crossformer(Exp_Basic):
    ...
    def test(self, setting, save_pred=True, inverse=False):
        ...
        # result save
        folder_path = './save_pred/' + setting + '/'

        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

        mae, mse, rmse, mape, mspe ,sse, sst= metrics_mean
        print('mse:{}, mae:{}, rmse:{}, mape:{}, mspe:{}, sse:{}, sst:{}, r2_score:{}'.format(mse, mae, rmse, mape, mspe, sse, sst, 1-sse/sst))

        # 将r2_score写入文件
        with open('r2_score_output.txt', 'w') as file:
            file.write(str(1-sse/sst))

        np.save(folder_path + 'metrics.npy', np.array([mae, mse, rmse, mape, mspe, sse, sst]))
        if (save_pred):
    ...
    def eval(self, setting, save_pred=False, inverse=False):
        ...
        mae, mse, rmse, mape, mspe ,sse, sst= metrics_mean
        print('mse:{}, mae:{}, rmse:{}, mape:{}, mspe:{}, sse:{}, sst:{}, r2_score:{}'.format(mae, mse, rmse, mape, mspe, sse, sst, 1-sse/sst))
        ...
        return mae, mse, rmse, mape, mspe, sse, sst